...
static const char * const g3a_numbered_sections[] =
{
  NULL, /* Custom section, different layout.  */
  G3A_SECTION ( 1, "type"),
  G3A_SECTION ( 2, "import"),
  G3A_SECTION ( 3, "function"),
  G3A_SECTION ( 4, "table"),
  G3A_SECTION ( 5, "memory"),
  G3A_SECTION ( 6, "global"),
  G3A_SECTION ( 7, "export"),
  G3A_SECTION ( 8, "start"),
  G3A_SECTION ( 9, "element"),
  G3A_SECTION (10, "code"),
  G3A_SECTION (11, "data"),
};

#define G3A_NUMBERED_SECTIONS ARRAY_SIZE (g3a_numbered_sections)

/* Resolve SECTION_CODE to a section name if there is one, NULL
   otherwise.  */

static const char *
g3a_section_code_to_name (bfd_byte section_code)
{
  if (section_code < G3A_NUMBERED_SECTIONS)
    return g3a_numbered_sections[section_code];
...
static unsigned int
g3a_section_name_to_code (const char *name)
{
  unsigned i;

  for (i = 1; i < G3A_NUMBERED_SECTIONS; i++)
    if (strcmp (name, g3a_numbered_sections[i]) == 0)
      return i;
...
static bool
g3a_write_uleb128 (bfd *abfd, bfd_vma v)
{
  do
    {
      bfd_byte c = v & 0x7f;
...
static bool
g3a_read_magic (bfd *abfd, bool *errorptr)
{
  bfd_byte magic_const[SIZEOF_G3A_MAGIC] = G3A_MAGIC;
  bfd_byte magic[SIZEOF_G3A_MAGIC];
...
static bool
g3a_read_version (bfd *abfd, bool *errorptr)
{
  bfd_byte vers_const[SIZEOF_G3A_VERSION] = G3A_VERSION;
  bfd_byte vers[SIZEOF_G3A_VERSION];
...
static bool
g3a_read_header (bfd *abfd, bool *errorptr)
{
  if (! g3a_read_magic (abfd, errorptr))
    return false;

  if (! g3a_read_version (abfd, errorptr))
    return false;
...
/* Scan the "function" subsection of the "name" section ASECT in the
   g3a module ABFD. Create symbols. Return TRUE on success.  */

static bool
g3a_scan_name_function_section (bfd *abfd, sec_ptr asect)
{
  bfd_byte *p;
  bfd_byte *end;
  ...
  while (p < end)
    {
      ...
      if (subsection_code == G3A_FUNCTION_SUBSECTION)
	break;
  ...
  space_function_index
    = bfd_make_section_with_flags (abfd, G3A_SECTION_FUNCTION_INDEX,
				   SEC_READONLY | SEC_CODE);

  if (!space_function_index)
    space_function_index
      = bfd_get_section_by_name (abfd, G3A_SECTION_FUNCTION_INDEX);
...
static int
g3a_read_byte (bfd *abfd, bool *errorptr)
{
  bfd_byte byte;
...
/* Scan the g3a module ABFD, creating sections and symbols.
...
g3a_scan (bfd *abfd)
{
  ...
  if (!g3a_read_header (abfd, &error))
    goto error_return;

  while ((section_code = g3a_read_byte (abfd, &error)) != EOF)
    {
      if (section_code != 0)
	{
	  const char *sname = g3a_section_code_to_name (section_code);
	  ...
	  bfdsec->size = g3a_read_leb128 (abfd, &error, &bytes_read, false);
	  if (error)
	    goto error_return;
	}
      else
	{
	  ...
	  bfd_vma namelen;
	  char *name;
	  char *prefix = G3A_SECTION_PREFIX;
	  size_t prefixlen = strlen (prefix);
	  ufile_ptr filesize;

	  payload_len = g3a_read_leb128 (abfd, &error, &bytes_read, false);
	  if (error)
	    goto error_return;
	  namelen = g3a_read_leb128 (abfd, &error, &bytes_read, false);
...
static void
g3a_register_section (bfd *abfd ATTRIBUTE_UNUSED,
		       asection *asect,
		       void *fsarg)
{
  sec_ptr *numbered_sections = fsarg;
  int idx = g3a_section_name_to_code (asect->name);
...
/* Compute the file position of ABFD's section ASECT.  FSARG is a
   ...
   We allow section names of the form .g3a.id to encode the numbered
...
static void
g3a_compute_custom_section_file_position (bfd *abfd,
					   sec_ptr asect,
					   void *fsarg)
{
  ...
  idx = g3a_section_name_to_code (asect->name);

  if (idx != 0)
    return;

  if (startswith (asect->name, G3A_SECTION_PREFIX))
    {
      const char *name = asect->name + strlen (G3A_SECTION_PREFIX);
      ...
      do
	{
      ...
      if (! g3a_write_uleb128 (abfd, 0)
	  || ! g3a_write_uleb128 (abfd, payload_len)
	  || ! g3a_write_uleb128 (abfd, name_len)
...
static bool
g3a_compute_section_file_positions (bfd *abfd)
{
  bfd_byte magic[SIZEOF_G3A_MAGIC] = G3A_MAGIC;
  bfd_byte vers[SIZEOF_G3A_VERSION] = G3A_VERSION;
  sec_ptr numbered_sections[G3A_NUMBERED_SECTIONS];
  ...
  for (i = 0; i < G3A_NUMBERED_SECTIONS; i++)
    numbered_sections[i] = NULL;

  bfd_map_over_sections (abfd, g3a_register_section, numbered_sections);
  ...
  for (i = 0; i < G3A_NUMBERED_SECTIONS; i++)
	...
	return false;
      if (! g3a_write_uleb128 (abfd, i)
	  || ! g3a_write_uleb128 (abfd, size))
	return false;
  ...
  bfd_map_over_sections (abfd, g3a_compute_custom_section_file_position, &fs);
...
static bool
g3a_set_section_contents (bfd *abfd,
			   sec_ptr section,
			   const void *location,
			   file_ptr offset,
			   bfd_size_type count)
{
  ...
  if (! abfd->output_has_begun
      && ! g3a_compute_section_file_positions (abfd))
    return false;
...
static bool
g3a_write_object_contents (bfd* abfd)
{
  bfd_byte magic[] = G3A_MAGIC;
  bfd_byte vers[] = G3A_VERSION;
...
static long
g3a_get_symtab_upper_bound (bfd *abfd)
{
  tdata_type *tdata = abfd->tdata.any;
...
static long
g3a_canonicalize_symtab (bfd *abfd, asymbol **alocation)
{
  tdata_type *tdata = abfd->tdata.any;
  size_t i;
...
static asymbol *
g3a_make_empty_symbol (bfd *abfd)
{
...
static void
g3a_get_symbol_info (bfd *abfd ATTRIBUTE_UNUSED,
		      asymbol *symbol,
		      symbol_info *ret)
{
...
g3a_object_p (bfd *abfd)
{
  ...
  if (!g3a_mkobject (abfd))
    return NULL;

  if (!g3a_scan (abfd)
      || !bfd_default_set_arch_mach (abfd, bfd_arch_sh, 0))
    {
  ...
  s = bfd_get_section_by_name (abfd, G3A_NAME_SECTION);
  if (s != NULL && g3a_scan_name_function_section (abfd, s))
    abfd->flags |= HAS_SYMS;
...
#define g3a_set_arch_mach		  _bfd_generic_set_arch_mach

/* BFD_JUMP_TABLE_SYMBOLS */
#define g3a_get_symbol_version_string	  _bfd_nosymbols_get_symbol_version_string
#define g3a_bfd_is_local_label_name	   bfd_generic_is_local_label_name
#define g3a_bfd_is_target_special_symbol _bfd_bool_bfd_asymbol_false
#define g3a_get_lineno			  _bfd_nosymbols_get_lineno
#define g3a_find_nearest_line		  _bfd_nosymbols_find_nearest_line
#define g3a_find_line			  _bfd_nosymbols_find_line
#define g3a_find_inliner_info		  _bfd_nosymbols_find_inliner_info
#define g3a_bfd_make_debug_symbol	  _bfd_nosymbols_bfd_make_debug_symbol
#define g3a_read_minisymbols		  _bfd_generic_read_minisymbols
#define g3a_minisymbol_to_symbol	  _bfd_generic_minisymbol_to_symbol

const bfd_target sh_g3a_prizm_vec =
{
  "g3a",			/* Name.  */
  ...
  BFD_JUMP_TABLE_SYMBOLS (g3a),
  BFD_JUMP_TABLE_RELOCS (_bfd_norelocs),
  BFD_JUMP_TABLE_WRITE (g3a),
